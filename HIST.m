...
src=load_nii('F:\python\Levelset\LevelSet2.0\brain data\1_brain.nii');
vol=src.img;
[height,width,depth]=size(vol);

parfor dataset=1:15
    name=num2str(dataset);
    src=load_nii(['F:\python\Levelset\LevelSet2.0\brain data\train\',name,'_brain.nii']);
    vol=src.img;
    [SLIC_label_train,sv_centers]=SLIC_3D(vol,SLIC_num,0,1,2);
    vol_int=round(vol/max(max(max(vol)))*255);
    src=load_nii(['F:\python\Levelset\LevelSet2.0\brain data\train\',name,'_truth.nii']);
    truth_train=src.img;
    ...
    for p=1:height
        for q=1:width
            for r=1:depth
                ...
                counter(label,truth)= counter(label,truth_train(p,q,r))+1;
                hist(label,vol_int(p,q,r))=hist(label,vol_int(p,q,r))+1;
            end
        end
    end
    temp=[sv_centers,hist];
...
%
src=load_nii(['F:\python\Levelset\LevelSet2.0\brain data\test\',name,'_truth.nii']);
truth_test=src.img;
...
%%
temp=squeeze(truth_test_pr(:,:,150));
DrawRGBMRI(temp);
